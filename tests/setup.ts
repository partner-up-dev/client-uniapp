...
vi.stubGlobal('uni', {
  ...
  request: vi.fn().mockImplementation((options: any) => {
    ...
    if (data) {
      if (method === 'GET') {
        // GET 请求，将 data 转换为 query string
        const params = new URLSearchParams()
        for (const [key, value] of Object.entries(data)) {
          params.append(key, String(value))
        }
        const separator = url.includes('?') ? '&' : '?'
        requestUrl = `${url}${separator}${params.toString()}`
      } else {
        // POST 等请求，处理 body
    ...
    // 转换 header 键为小写，因为 fetch headers 是大小写不敏感的，但为了兼容性
    ...
    fetch(requestUrl, {
      ...
      .then(async (response) => {
        ...
        const result = {
          ...
          cookies: [] // fetch 不直接支持 cookies，这里简化
        }
      ...
      .catch((error) => {
        ...
        if (error.name === 'AbortError') {
          ...
          fail?.({ errMsg: `request:fail ${error.message}` })
        }
        complete?.()
      })
  ...
  // 设置 getMenuButtonBoundingClientRect 方法
