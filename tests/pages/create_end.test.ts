...
describe("create_end page form functionality", () => {
  ...
  it("should validate title length constraints", () => {
    // Test title length validation logic (3-12 characters per schema)
    const validateTitleLength = (title: string | null): boolean => {
      if (!title) return true; // null is valid
      return title.length >= 3 && title.length <= 12;
    };
    
    expect(validateTitleLength(null)).toBe(true);
    expect(validateTitleLength("测试标题")).toBe(true); // 4 chars, valid
    expect(validateTitleLength("短")).toBe(false); // 1 char, too short
    expect(validateTitleLength("这是一个非常长的标题")).toBe(true); // 11 chars, valid
    expect(validateTitleLength("这是一个非常长的标题超过了")).toBe(false); // 14 chars, too long
  });
