...
| Chat with partners | `src/pages/communication/chat.vue`, `src/store/communication/chat.ts`, `src/store/communication/message.ts`, `src/business/communication/` | Message store authority, unread state, auth headers | Unit TDD exists for message store; automated tests are not documented yet. |
| View notifications | `src/pages/notification/notification.vue`, `src/store/communication/message.ts`, `src/store/communication/chat.ts`, `src/utils/tabbar.ts` | Unread state, tabbar state, message event propagation | Unit TDD exists for message store; tabbar badge behavior needs explicit verification when changed. |
