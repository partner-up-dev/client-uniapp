...
import { geoElementProps, geoElementEmits, GeoElementType } from "./GeoElement";
...
<template>
  <view v-if="element" class="geo-element">
    <!-- 地理元素预览 -->
    <view class="preview">
      <RoutePreview v-if="element.type === GeoElementType.Route" :route="element.value" />

      <template v-else-if="element.type === GeoElementType.POI">
        <!-- POI 预览 -->
        <view class="poi-item">
    ...
    <view class="item-type">
      <text
        v-if="element.type === GeoElementType.Route"
        class="i-mdi-directions item-type-icon"
      ></text>
      <text
        v-else-if="element.type === GeoElementType.POI"
