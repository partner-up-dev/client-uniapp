<template>
  <view class="pu-map" :style="{ height }">
    <!-- UniApp地图组件 -->
    <map :id="mapId" ref="mapRef" class="map-view" :latitude="currentCenter?.latitude"
      :longitude="currentCenter?.longitude" :markers="markers" :polyline="polylines" :show-location="showLocation"
      :enable-3d="enable3d" :show-compass="showCompass" :show-scale="showScale" :enable-scroll="enableScroll"
      :enable-zoom="enableZoom" @markertap="onMarkerClick" @tap="onMapClick" />
...
<style lang="scss">
.pu-map-operation {
  @include pu-icon("medium", true);

  background-color: sys-var(color, surface-container);
  border-radius: sys-var(radius, xs);
  box-shadow: sys-var(shadow, 1);
  color: sys-var(color, on-surface);

  &:active {
    background-color: sys-var(color, surface-container-low);
  }
}
</style>